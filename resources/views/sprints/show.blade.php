        ...
        <div class="flex gap-2">
            @if (!in_array($sprint->status?->key, ['closed', 'archived'], true))
                <form method="POST" action="{{ route('sprints.complete', $sprint) }}"
                      onsubmit="return confirm('Complete this sprint? Unfinished stories will move to the next draft/ready sprint (or a carryover sprint will be created).');">
                    @csrf
                    <button type="submit"
            ...
            @unless ($sprint->is_frozen)
                <div class="relative group">
                    <form method="POST" action="{{ route('sprints.freeze', $sprint) }}" 
                        ...
                        @csrf
                        <button type="submit"
                                title="Marks sprint Ready and locks story membership. Board movement/status updates remain available."
                    ...
                    <div class="pointer-events-none absolute right-0 top-full mt-2 z-20 hidden w-72 rounded-md bg-gray-900 px-3 py-2 text-xs text-white shadow-lg group-hover:block group-focus-within:block">
                        Marks sprint <strong>Ready</strong> and locks story membership. You can still move stories across board columns while executing.
                    </div>
                </div>
